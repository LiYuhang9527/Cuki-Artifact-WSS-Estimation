...
public class BitMapWithClockSketch<T> implements Serializable {
     ...
     */
    public BitMapWithClockSketch(int windowSize, int hashNum, int bitsPerClock, int clockWidth, int bitsPerSize , Funnel<? super T> funnel){
    ...
    public static <T> BitMapWithClockSketch<T> create(int windowSize, int memory, int hashNum, int bitsPerClock, int bitsPerSize, Funnel<?super T>funnel){
        return new BitMapWithClockSketch<T>(windowSize,hashNum,bitsPerClock,memory/(bitsPerClock+bitsPerSize),bitsPerSize,funnel);
    }
    public void put(T trace,int size){
        queryNum++;
        for(HashFunction hashFunc:hashFuncs){
            int pos = Math.abs(hashFunc.newHasher().putObject(trace,mFunnel).hash().asInt()%mClockWidth);
            if(clock[pos]==0){
                missNum++;
                traceSize[pos] = size;
