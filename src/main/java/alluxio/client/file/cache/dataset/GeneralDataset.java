...
public class GeneralDataset<T> implements Dataset<T> {
    ...
    public DatasetEntry<T> next() {
        ...
        if (cnt != null && cnt > 0) {
            ...
            scopeToSize.put(entry.getScopeInfo(), scopeToSize.getOrDefault(entry.getScopeInfo(), 0) + entry.getSize());
            scopeToNumber.put(entry.getScopeInfo(), scopeToNumber.getOrDefault(entry.getScopeInfo(), 0) + 1);
        ...
        if (queue.size() > windowSize) {
            ...
            if (itemCount == 1) {
                ...
                scopeToSize.put(staleItem.getScopeInfo(), scopeToSize.getOrDefault(staleItem.getScopeInfo(), 0) - staleItem.getSize());
                scopeToNumber.put(staleItem.getScopeInfo(), scopeToNumber.getOrDefault(staleItem.getScopeInfo(), 0) - 1);
