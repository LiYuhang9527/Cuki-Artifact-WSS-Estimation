...
public class ScopedClockCuckooFilter<T> implements Serializable {
  ...
  public ScopedClockCuckooFilter(CuckooTable table, CuckooTable clockTable, CuckooTable sizeTable,
      CuckooTable scopeTable, Funnel<? super T> funnel, HashFunction hasher) {
    this.table = table;
    ...
    this.bitsPerSize = sizeTable.getBitsPerTag();
    this.scopeTable = scopeTable;
    this.bitsPerScope = scopeTable.getBitsPerTag();
    this.funnel = funnel;
    this.hasher = hasher;
  ...
  public static <T> ScopedClockCuckooFilter<T> create(Funnel<? super T> funnel,
      long expectedInsertions, int bitsPerClock, int bitsPerSize, int bitsPerScope, double fpp,
  ...
  public static <T> ScopedClockCuckooFilter<T> create(Funnel<? super T> funnel,
      long expectedInsertions, int bitsPerClock, int bitsPerSize, int bitsPerScope, double fpp,
      double loadFactor) {
    return create(funnel, expectedInsertions, bitsPerClock, bitsPerSize, bitsPerScope, fpp,
        loadFactor, Hashing.murmur3_128());
  }
  ...
  public int size(CacheScope scopeInfo) {
    int scope = encodeScope(scopeInfo);
  ...
  public int sizeInBytes(CacheScope scopeInfo) {
    int scope = encodeScope(scopeInfo);
