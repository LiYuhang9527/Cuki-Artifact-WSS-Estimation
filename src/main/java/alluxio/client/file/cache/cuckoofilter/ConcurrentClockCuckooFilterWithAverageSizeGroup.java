...
import alluxio.client.file.cache.cuckoofilter.size.AverageSizeEncoder;
...
public class ConcurrentClockCuckooFilterWithAverageSizeGroup<T>
    implements ClockCuckooFilter<T>, Serializable {
  ...
  // size group
  private final ISizeEncoder[] mScopeToSizeEncoder;
  ...
  // size groups
  // private ISizeGroup[] mSizeGroups;
  ...
  public boolean put(T item, int size, CacheScope scopeInfo) {
    ...
    if (done && pos.mStatus == CuckooStatus.OK) {
      ...
      // convert size to size group
      int sizeGroup = mScopeToSizeEncoder[scope].encode(size);
      mScopeToSizeEncoder[scope].add(size);
      ...
      mSizeTable.writeTag(pos.getBucketIndex(), pos.getTagIndex(), sizeGroup);
      // update statistics
  ...
  public boolean mightContainAndResetClock(T item, int size, CacheScope scopeInfo) {
    return mightContainAndResetClockInternal(item, size, scopeInfo);
  }
   ...
   * @param item the item to be checked
   * @param size the size
   * @param scopeInfo the scope
   * @return true if item is in cuckoo filter; false otherwise
   */
  private boolean mightContainAndResetClockInternal(T item, int size, CacheScope scopeInfo) {
    boolean found;
    IndexAndTag indexAndTag = generateIndexAndTag(item);
    int b1 = indexAndTag.mBucket;
    int tag = indexAndTag.mTag;
  ...
  public int getItemSize(CacheScope scopeInfo) {
    int scope = encodeScope(scopeInfo);
    return (int) mScopeToSizeEncoder[scope].getTotalSize();
  }
  ...
  private int agingBucket(int b) {
    ...
    for (int j = 0; j < TAGS_PER_BUCKET; j++) {
      ...
      if (oldClock > 0) {
        ...
        int sizeGroup = mSizeTable.readTag(b, j);
        int size = mScopeToSizeEncoder[scope].dec(sizeGroup);
        updateScopeStatistics(scope, -1, -size);
        mTotalBytes.addAndGet(-size);
      }
